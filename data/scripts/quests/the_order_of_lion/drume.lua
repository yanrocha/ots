...
function drumeAction.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	for x, creature in pairs(spectators) do
	...
	local tempMonster
	for x, pos in pairs(config.lionPosition) do
		tempMonster = Game.createMonster("Lion Commander", pos, true, true)
	...
	for x, pos in pairs(config.usurperPosition) do
		tempMonster = Game.createMonster("Usurper Commander", pos, true, true)
		if not tempMonster then
			player:sendCancelMessage("There was an error, contact an admin.")
			player:getPosition():sendMagicEffect(CONST_ME_POFF)
			return true
